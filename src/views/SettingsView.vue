...
const permissionDescriptions: Record<string, string> = {
  ...
  stage:
    "允许把构建产物上传到中转目录（后端含同步备机），但不允许替换线上、回滚、Docker 部署。线上替换仍需人在软件里确认，推荐给 AI 用。",
...
const mcpJsonSnippet = computed(() => {
  ...
  return JSON.stringify(
    {
      mcpServers: {
        "jy-deploy": { url: `http://127.0.0.1:${port}/mcp` },
      },
    },
    null,
    2
...
async function exportConfigFile() {
  const targetPath = await saveDialog({
    title: "导出配置",
    defaultPath: "jy-deploy-config.json",
...
async function importConfigFile() {
  ...
  await ElMessageBox.confirm(
    "导入将覆盖当前全部配置（服务器、隧道、部署映射），确认继续？",
    "导入确认",
    { type: "warning", confirmButtonText: "导入并覆盖" }
  );
  try {
...
<template>
  <div v-if="config">
    ...
    <el-card class="mcp-card" shadow="never">
      <template #header>
        <div class="mcp-header">
          ...
          <el-tag v-if="mcpRunningPort" type="success">
            运行中 · 127.0.0.1:{{ mcpRunningPort }}
          </el-tag>
      ...
      <el-form label-width="130px">
        <el-form-item label="启用 MCP 服务">
          ...
          <span class="form-hint">
            只监听本机 127.0.0.1，AI（如 Cursor）构建完包后可直接调用本工具发起部署
          </span>
        </el-form-item>
          ...
          <div style="width: 100%">
            ...
            <template v-if="targetScope === 'custom'">
              <div class="scope-row">
                ...
                <el-select
                  v-model="config.mcp.allowedBackendGroupIds"
                  multiple
                  collapse-tags
                  placeholder="不选择 = 全部禁止"
                  style="flex: 1"
                >
                  <el-option
              ...
              <div class="scope-row">
                ...
                <el-select
                  v-model="config.mcp.allowedFrontendTargetIds"
                  multiple
                  collapse-tags
                  placeholder="不选择 = 全部禁止"
                  style="flex: 1"
                >
                  <el-option
                    v-for="target in config.frontendTargets"
                    :key="target.id"
                    :label="target.name"
                    :value="target.id"
                  />
              ...
              <div class="scope-row">
                ...
                <el-select
                  v-model="config.mcp.allowedDockerTargetIds"
                  multiple
                  collapse-tags
                  placeholder="不选择 = 全部禁止"
                  style="flex: 1"
                >
                  <el-option
                    v-for="target in config.dockerTargets"
                    :key="target.id"
                    :label="target.name"
                    :value="target.id"
                  />
        ...
        <el-form-item label="客户端接入配置">
          <div style="width: 100%">
            ...
            <el-button size="small" @click="copyMcpSnippet">
              复制（粘贴到 Cursor 的 mcp.json）
            </el-button>
          </div>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="saveMcpSettings">
            保存 MCP 设置
          </el-button>
        </el-form-item>
      </el-form>
    ...
    <el-alert
      type="warning"
      :closable="false"
      title="安全提示：配置中的密码仅保存在本机配置文件中。建议服务器尽量使用私钥认证（在服务器编辑里选择「私钥」）"
    />
  </div>
</template>
