  ...
  <div class="help-view" :class="{ embedded }">
    ...
    <el-collapse v-model="activeSections">
      ...
      <el-collapse-item title="四、SSH 终端 / SFTP / 远程桌面" name="terminal">
        <div class="help-body">
          <ul>
            <li><b>SSH 终端</b>：选服务器 → 新建会话，支持多标签。连 Windows 服务器默认进 PowerShell；若要 cmd，输入 <code>cmd</code> 回车即可。标签可弹出独立窗口，关掉弹出窗不会断开会话。</li>
            <li><b>lrzsz 文件传输</b>：在 SSH 终端中直接输入 <code>rz</code> 回车即可弹出本地文件选择器上传文件到远端服务器；输入 <code>sz 文件名</code> 回车即可弹出保存对话框将远端文件下载到本地，支持传输进度与取消。</li>
      ...
      <el-collapse-item title="五、后端部署（重点）" name="backend">
        <div class="help-body">
          <p>右上角可添加负载组；打开下拉列表后，把鼠标放到某一组上会出现上移、下移和删除。组内项目和服务器在「项目配置」里改。</p>
          <p><b>三种部署方式：</b></p>
          <ul>
      ...
      <el-collapse-item title="六、前端部署 / Docker 部署" name="frontend">
        <div class="help-body">
          <ul>
            <li><b>前端部署</b>：本地 dist 打包后一次上传到服务器再解压，避免逐文件 SFTP。项目按环境分组（默认有开发 / 正式，右上角可添加；打开下拉后把鼠标放到某一环境上可上移、下移或删除），顶部切换环境后再部署，避免发错。直接替换或从中转替换时会做发布前快照，可在「发布历史」点<b>回滚</b>恢复；替换成功后删除中转目录。历史可按环境筛选，记录可删除。Windows / Linux 都能部署。同样支持「仅上传到中转」和「从中转替换」。</li>
