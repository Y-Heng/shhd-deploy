  ...
  <div class="help-view" :class="{ embedded }">
    ...
    <el-collapse v-model="activeSections">
      ...
      <el-collapse-item title="五、后端部署（重点）" name="backend">
        <div class="help-body">
          <p>右上角可添加、删除负载组，并用上移/下移调整下拉顺序。组内项目和服务器在「项目配置」里改。</p>
          <p><b>三种部署方式：</b></p>
          <ul>
      ...
      <el-collapse-item title="六、前端部署 / Docker 部署" name="frontend">
        <div class="help-body">
          <ul>
            <li><b>前端部署</b>：本地 dist 打包后一次上传到服务器再解压，避免逐文件 SFTP。项目按环境分组（默认有开发 / 正式，右上角可添加、删除，也可用上移/下移调整顺序），顶部切换环境后再部署，避免发错。直接替换或从中转替换时会做发布前快照，可在「发布历史」点<b>回滚</b>恢复。历史可按环境筛选，记录可删除。Windows / Linux 都能部署。同样支持「仅上传到中转」和「从中转替换」。</li>
