      ...
      <el-collapse-item title="五、后端部署（重点）" name="backend">
        <div class="help-body">
          ...
          <ul>
            ...
            <li>替换前自动备份目标目录；「附加备份」勾选后还会把整个应用目录复制成 <code>目录名-日期</code>。本地与线上目录一比一覆盖，不删除线上其它文件。项目里可配置忽略规则和白名单（可直接选文件/文件夹）。部署时按「改动起始日」只上传该日及之后改过的文件，每次进入部署页默认今天。部署前可点「预览文件」，在树里勾选本次要上传的文件。</li>
