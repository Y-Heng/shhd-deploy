  ...
  <div class="help-view" :class="{ embedded }">
    ...
    <el-collapse v-model="activeSections">
      ...
      <el-collapse-item title="六、前端部署 / Docker 部署" name="frontend">
        <div class="help-body">
          <ul>
            <li><b>前端部署</b>：本地 dist 打包后一次上传到服务器再解压，避免逐文件 SFTP。项目按环境分组（默认有开发 / 正式，右上角可添加；打开下拉后把鼠标放到某一环境上可上移、下移或删除），顶部切换环境后再部署，避免发错。直接替换或从中转替换时会做发布前快照，可在「发布历史」点<b>回滚</b>恢复。历史可按环境筛选，记录可删除。Windows / Linux 都能部署。同样支持「仅上传到中转」和「从中转替换」。</li>
