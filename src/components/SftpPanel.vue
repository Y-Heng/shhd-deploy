...
const localBreadcrumbs = computed(() => {
  if (isDriveListPath(localPath.value) || !localPath.value) return [{ label: '此电脑', path: LOCAL_DRIVES_PATH }]
...
function compareEntries<T extends { name: string; isDir: boolean; size: number; mtime: number; hidden?: boolean }>(left: T, right: T, sortKey: SortKey) {
...
const sortedLocalEntries = computed(() => [...localEntries.value].sort((left, right) => compareEntries(left, right, localSortKey.value)))

const sortedRemoteEntries = computed(() => [...remoteEntries.value].sort((left, right) => compareEntries(left, right, remoteSortKey.value)))
...
async function addShortcut(side: ShortcutSide, scope: ShortcutScope, path: string) {
  ...
  const currentList = side === 'local' ? (scope === 'public' ? publicLocalShortcuts.value : dedicatedLocalShortcuts.value) : scope === 'public' ? publicRemoteShortcuts.value : dedicatedRemoteShortcuts.value
  try {
...
async function addShortcutFromEntry(side: ShortcutSide, scope: ShortcutScope, entry: SftpEntry | LocalDirEntry | null) {
...
function findEntryPathFromRow(rowElement: HTMLElement | null) {
  ...
  return rowElement.getAttribute('data-row-key') || rowElement.querySelector('[data-path]')?.getAttribute('data-path') || ''
}

function rectsIntersect(left: { left: number; top: number; right: number; bottom: number }, right: { left: number; top: number; right: number; bottom: number }) {
...
function updateRubberSelection(side: PaneSide) {
  ...
  if (side === 'local') {
    ...
    rowNodes.forEach(node => {
      ...
      if (rectsIntersect(bandRect, { left: box.left, top: box.top, right: box.right, bottom: box.bottom })) hit.push(entry)
    })
...
function paneSideAtPoint(clientX: number, clientY: number): PaneSide | null {
  ...
  if (remoteBox && clientX >= remoteBox.left && clientX <= remoteBox.right && clientY >= remoteBox.top && clientY <= remoteBox.bottom) return 'remote'
  const localBox = localPaneRef.value?.getBoundingClientRect()
  if (localBox && clientX >= localBox.left && clientX <= localBox.right && clientY >= localBox.top && clientY <= localBox.bottom) return 'local'
  return null
}
...
async function finishCustomDrag(hoverSide: PaneSide | null, source: PaneSide, localRows: LocalDirEntry[], remoteRows: SftpEntry[]) {
...
async function onPointerUp(event: MouseEvent) {
  ...
  if (deferSingleSelect && !didPointerDrag && !rubberBand.value.active && !wasCustomDrag) {
...
async function uploadSelectedLocal() {
  const targets = localSelectedRows.value.length ? localSelectedRows.value : localSelectedPath.value ? localEntries.value.filter(item => item.path === localSelectedPath.value) : []
  if (!targets.length) {
...
async function createRemoteFolder() {
  ...
  if (!value) return
  const remoteDir = remotePath.value.endsWith('/') ? `${remotePath.value}${value}` : `${remotePath.value}/${value}`
  busy.value = true
  try {
...
async function removeRemoteSelected(entry?: SftpEntry) {
  ...
  const preview = targets.length === 1 ? `确认删除「${targets[0].name}」？${targets[0].isDir ? '（目录将递归删除）' : ''}` : `确认删除选中的 ${targets.length} 项？（目录将递归删除）`
...
async function renameRemoteSelected(entry?: SftpEntry) {
  const target = entry || remoteEntries.value.find(item => item.path === remoteSelectedPath.value) || remoteSelectedRows.value[0]
  if (!target) {
...
async function viewRemoteEntry(entry?: SftpEntry) {
  ...
  await ElMessageBox.alert([`名称：${target.name}`, `路径：${target.path}`, `类型：${target.isDir ? '目录' : '文件'}`, `大小：${formatSize(target.size, target.isDir)}`, `修改时间：${formatTime(target.mtime)}`].join('\n'), '查看属性', { confirmButtonText: '关闭' })
}
...
function cycleSortKey(side: PaneSide) {
  ...
  if (side === 'local') {
    ...
    ElMessage.success(`本地排序：${localSortKey.value === 'name' ? '名称' : localSortKey.value === 'size' ? '大小' : '修改时间'}`)
    return
  }
  const index = order.indexOf(remoteSortKey.value)
  remoteSortKey.value = order[(index + 1) % order.length]
  ElMessage.success(`远端排序：${remoteSortKey.value === 'name' ? '名称' : remoteSortKey.value === 'size' ? '大小' : '修改时间'}`)
}
...
async function onContextCommand(command: string) {
  ...
  if (side === 'local') {
    ...
    else if (command === 'mkdir' || command === 'rename' || command === 'remove') ElMessage.info('本地新建/重命名/删除请使用系统资源管理器')
    return
  }
...
async function isPointInsideElement(element: HTMLElement | null, position: { x: number; y: number }) {
  ...
  return points.some(point => point.x >= rect.left && point.x <= rect.right && point.y >= rect.top && point.y <= rect.bottom)
}
...
function onLocalRowDragStart(event: DragEvent, row: LocalDirEntry) {
  ...
  const selected = localSelectedRows.value.some(item => item.path === row.path) ? localSelectedRows.value : [row]
  if (!localSelectedRows.value.some(item => item.path === row.path)) setLocalSelection(selected, row.path)
...
function onRemoteRowDragStart(event: DragEvent, row: SftpEntry) {
  ...
  const selected = remoteSelectedRows.value.some(item => item.path === row.path) ? remoteSelectedRows.value : [row]
  if (!remoteSelectedRows.value.some(item => item.path === row.path)) setRemoteSelection(selected, row.path)
...
<template>
  <div class="sftp-panel">
    ...
    <div class="dual-panes">
      <!-- 左侧：本地 -->
      <div ref="localPaneRef" class="pane local-pane" :class="{ 'is-dragover': localDragOver }" @dragenter="onLocalPaneDragEnter" @dragover="onLocalPaneDragOver" @dragleave="onLocalPaneDragLeave" @drop="onLocalPaneDrop">
        <div class="pane-toolbar">
          <span class="pane-title">本地</span>
          <div class="pane-toolbar-actions">
            <el-button size="small" :icon="ArrowUp" :disabled="!canGoLocalParent" @click="goLocalParent"> 上级 </el-button>
            <el-button size="small" @click="loadLocalDir(LOCAL_DRIVES_PATH)">磁盘</el-button>
            <el-button size="small" :icon="Refresh" :loading="localLoading" @click="loadLocalDir(localPath)"> 刷新 </el-button>
          </div>
        </div>
        <div class="shortcut-bar">
          <div v-for="group in localShortcutGroups" :key="'local-' + group.scope" class="shortcut-group">
            <span class="shortcut-group-label">{{ group.label }}</span>
            <button v-for="shortcut in group.paths" :key="group.scope + shortcut" type="button" class="shortcut-chip" :class="{ 'is-dedicated': group.scope === 'dedicated' }" :title="shortcut" @click="openShortcut('local', shortcut)">
              <span class="shortcut-chip-label">{{ shortcutLabel(shortcut) }}</span>
              <span class="shortcut-chip-remove" title="删除指定" @click.stop="removeShortcut('local', group.scope, shortcut)">×</span>
            </button>
            <button type="button" class="shortcut-add" :title="group.scope === 'public' ? '添加当前路径为公共指定' : '添加当前路径为专属指定'" @click="addCurrentShortcut('local', group.scope)">
        ...
        <div class="sftp-pathbar">
          <div class="crumbs">
            <button v-for="(crumb, index) in localBreadcrumbs" :key="crumb.path + crumb.label" type="button" class="crumb" @click="loadLocalDir(crumb.path)">
        ...
        <div ref="localTableWrapRef" class="table-wrap" @mousedown="onTableWrapMouseDown('local', $event)" @contextmenu.capture.prevent="onTableContextMenu('local', $event)">
          <el-table ref="localTableRef" v-loading="localLoading" :data="sortedLocalEntries" height="100%" class="sftp-table" row-key="path" :row-class-name="getLocalRowClassName" @row-dblclick="onLocalRowDblClick">
      ...
      <!-- 右侧：远端 -->
      <div ref="remotePaneRef" class="pane remote-pane" :class="{ 'is-dragover': remoteDragOver }" @dragenter="onRemotePaneDragEnter" @dragover="onRemotePaneDragOver" @dragleave="onRemotePaneDragLeave" @drop="onRemotePaneDrop">
        <div class="pane-toolbar">
          <span class="pane-title">{{ serverName || '远端' }}</span>
          <div class="pane-toolbar-actions">
            <el-button size="small" :icon="ArrowUp" :disabled="remotePath === '/'" @click="goRemoteParent"> 上级 </el-button>
            <el-button size="small" :icon="Refresh" :loading="remoteLoading" @click="loadRemoteDir(remotePath)"> 刷新 </el-button>
          </div>
        </div>
        <div class="shortcut-bar">
          <div v-for="group in remoteShortcutGroups" :key="'remote-' + group.scope" class="shortcut-group">
            <span class="shortcut-group-label">{{ group.label }}</span>
            <button v-for="shortcut in group.paths" :key="group.scope + shortcut" type="button" class="shortcut-chip" :class="{ 'is-dedicated': group.scope === 'dedicated' }" :title="shortcut" @click="openShortcut('remote', shortcut)">
              <span class="shortcut-chip-label">{{ shortcutLabel(shortcut) }}</span>
              <span class="shortcut-chip-remove" title="删除指定" @click.stop="removeShortcut('remote', group.scope, shortcut)">×</span>
            </button>
            <button type="button" class="shortcut-add" :title="group.scope === 'public' ? '添加当前路径为公共指定' : '添加当前路径为专属指定'" @click="addCurrentShortcut('remote', group.scope)">
              <el-icon><Plus /></el-icon>
            </button>
          </div>
        </div>
        <div class="sftp-pathbar">
          <div class="crumbs">
            <button v-for="(crumb, index) in remoteBreadcrumbs" :key="crumb.path" type="button" class="crumb" @click="loadRemoteDir(crumb.path)">
              <span v-if="index > 0" class="crumb-sep">/</span>
    ...
    <Teleport to="body">
      <div v-if="rubberBand.active" class="rubber-band" :style="rubberBandStyle" />
      <div v-if="customDrag" class="sftp-drag-ghost" :style="{ left: `${customDrag.x}px`, top: `${customDrag.y}px` }">
        {{ customDrag.label }}
      </div>
      <div v-if="contextMenu.visible" class="ctx-menu" :style="{ left: `${contextMenu.x}px`, top: `${contextMenu.y}px` }" @click.stop @contextmenu.prevent>
        <template v-if="contextMenu.side === 'local'">
          <button v-if="contextHasEntry && contextIsDir" type="button" class="ctx-item" @click="onContextCommand('open')">打开</button>
          <button v-if="contextHasEntry" type="button" class="ctx-item" @click="onContextCommand('upload')">上传到远端</button>
        ...
        <template v-else>
          <button v-if="contextHasEntry && contextIsDir" type="button" class="ctx-item" @click="onContextCommand('open')">打开</button>
          <button v-if="contextHasEntry" type="button" class="ctx-item" @click="onContextCommand('download')">下载到左侧</button>
          <button type="button" class="ctx-item" @click="onContextCommand('mkdir')">新建文件夹</button>
          <button v-if="contextHasEntry" type="button" class="ctx-item" @click="onContextCommand('rename')">重命名</button>
          <button v-if="contextHasEntry" type="button" class="ctx-item danger" @click="onContextCommand('remove')">删除</button>
          <button v-if="contextHasEntry" type="button" class="ctx-item" @click="onContextCommand('view')">查看</button>
          <button type="button" class="ctx-item" @click="onContextCommand('shortcut-public')">添加为公共指定</button>
          <button type="button" class="ctx-item" @click="onContextCommand('shortcut-dedicated')">添加为专属指定</button>
          <button type="button" class="ctx-item" @click="onContextCommand('refresh')">刷新</button>
          <div class="ctx-sep" />
